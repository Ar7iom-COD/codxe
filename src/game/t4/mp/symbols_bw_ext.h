...
// r307: SV_DropClient — Ghidra-verified on TU7 via EXE_PLAYERKICKED xref chain.
// FUN_82283BF0 fingerprints: state==CS_ZOMBIE guard at top, name read from
// cl+0x21328, clientNum compute via stride 0xB762C from svs.clients at
// DAT_830c0c90, EXE_DISCONNECTED / EXE_PLAYERKICKED_BAN string branches, sets
// cl->state = CS_ZOMBIE before return. Signature (client_t*, const char*, bool).
// Old base-T4 address happened to land at the same place on TU7 — no relocation.
typedef void (*SV_DropClient_BW_t)(clientBW_t *cl, const char *reason, int tellThem);
static SV_DropClient_BW_t SV_DropClient_BW =
    reinterpret_cast<SV_DropClient_BW_t>(0x82283BF0);
